...
LOG = "bot_mensual.log"
EXCEL_HIST = "Historial_Operaciones_Mensual.xlsx"
ESTADO_BOT = "estado_bot_mensual.json"
...
def recoger(opts):
    ...
    vistos_n = {}
    for nombre, fn in (("jina", rt.descargar_jina_x), ("xcancel", rt.descargar_xcancel)):
        try:
            p = fn()
            if p:
                vistos_n.update(p)
    ...
    if not vistos_n:
        ...
        try:
            notificar.alerta_error(
                "⚠️ No se pudieron recoger tweets de @elonmusk (jina y xcancel "
...
def actualizar_mercado():
    ...
    try:
        ...
        abiertos = [m for m in mks if not m["cerrado"] and m["tipo"] == "mensual"]
        log(f"      · {len(mks)} mercados · {len(abiertos)} mensuales abiertos "
...
def trading(opts):
    ...
    if opts.modo == "real":
        ...
        try:
            import operar_real_mensual as operar_real
...
def avisar_casi_senal():
    """Avisa al móvil si hay un mercado de la ventana cerca de cumplir las reglas
...
def pasada(opts):
    t0 = time.time()
    log("=" * 62)
    log("PASADA COMPLETA (MENSUAL)" + (f"  ·  MODO: {opts.modo.upper()}" if opts.modo == "real" else ""))
    recoger(opts)
    actualizar_mercado()
    trading(opts)
    avisar_casi_senal()
...
def ver_estado():
    ...
    try:
        ...
        if est.get("activa"):
            ...
            print("  Sin apuesta activa (esperando señal con ventaja ≥ 10pp y cuota ≥ 3.00)")
        if est["historial"]:
