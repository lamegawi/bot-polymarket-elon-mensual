...
ESTADO = "real_mensual.json"
HISTORIAL = "resultados_real_mensual.csv"
BANKROLL = 500.0
...
def verificar_saldo_usdc(client):
    ...
    IMPORTANTE (Polymarket V2): este es el saldo REAL de trading (collateral
    pUSD dentro del CLOB). En cuentas con deposit wallet, get_balance_allowance
    devuelve el balance del funder correctamente (verificado: el saldo
    depositado aparece aquí, no suelto on-chain en la wallet)."""
    try:
...
def resolver(estado, fee_pct=0.0):
    ...
    try:
        notificar.enviar(
            ...
            titulo="[MENSUAL] 💰 Apuesta REAL cerrada",
...
def abrir(estado, dry=False, actualizar=False):
    ...
    candidatas = [c for c in candidatas if c["tipo"] == "mensual"]
    if not candidatas:
        print(f"  (sin señal mensual → no se abre apuesta real · paso {estado['paso']})")
        return False
        ...
        try:
            ...
            try:
                notificar.enviar(
                    ...
                    f"{saldo_ntfy.saldo_real_texto()}",
                    titulo="[MENSUAL] 💰 Apuesta REAL abierta",
                    etiqueta="moneybag")
            except Exception as e:
...
def pasada_real(dry=False, actualizar=False, excel=False):
    ...
    print(f"[{datetime.now(ET).strftime('%Y-%m-%d %H:%M %Z')}] Trading REAL MENSUAL "
          f"{'(SECO)' if dry else ''} · saldo ${estado['saldo']:.2f} · "
          f"paso {estado['paso']}")
    # vigilar órdenes pendientes de llenado (sin bloquear el bot)
    if not dry:
...
def probar_orden():
    ...
    mercado mensual activo — es un precio imposible (los bins cotizan ≥ 0.10),
    ...
    activo = next((m for m in mercados
                   if not m["cerrado"] and m["tipo"] == "mensual"
                   and m.get("fin_iso")
                   and datetime.fromisoformat(m["fin_iso"]) > ahora), None)

    if not activo:
        print("  (no hay mercado mensual abierto ahora mismo)")
        return
